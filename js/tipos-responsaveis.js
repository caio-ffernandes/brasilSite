...
function saveTipoResp() {
    ...
    const url = id ? `http://10.188.35.101:8000/tipos-responsaveis/${id}` : `http://10.188.35.101:8000/tipos-responsaveis`;
...
function deleteTipoResp(id) {
    fetch(`http://10.188.35.101:8000/tipos-responsaveis/${id}`, {
        method: 'DELETE'
    })
    .then(response => {
        if (!response.ok) {
