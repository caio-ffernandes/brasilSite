    ...
    fetch('http://10.188.35.101:8000/posts')
        ...
        .then(data => {
            ...
            data.posts.forEach(post => {
                ...
                const imageUrl = nomeReal ? `http://10.188.35.101:8000/postsimg/${nomeReal}` : 'path/to/default-image.jpg'; // Caminho para uma imagem padrão
...
function fetchSubcategorias() {
    fetch('http://10.188.35.101:8000/subcategorias')
        .then(response => response.json())
        .then(data => {
...
function showEditPostForm(id, nome, descricao, imagem, subcategoria) {
    ...
    const imageUrl = `http://10.188.35.101:8000/posts/${imagem.split('/').pop()}`;
...
function savePost() {
    ...
    const url = id ? `http://10.188.35.101:8000/posts/${id}` : `http://10.188.35.101:8000/posts`;
...
function deletePost(id) {
    fetch(`http://10.188.35.101:8000/posts/${id}`, {
        method: 'DELETE'
    })
    .then(response => {
        if (!response.ok) {
